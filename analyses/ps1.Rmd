...
d$Input.price2 <- as.numeric(gsub(",", "", d$Input.price2)) # TODO: Compress two lines?
str(d)
...
3. Change inputs & responses to numeric
...
d.tidy <- select(d, WorkerId, Input.condition,
                 ...
                 Answer.plasma_cost, Answer.sushi_cost)
```
...
```{r rename}
d.tidy %>% rename(c('worker_id', 'condition', 'input_1', 'input_2', 'input_3', 'answer_dog',
                      'answer_plasma', 'answer_sushi'))
```
...
```{r gather}
d.tidy %>% 
  gather(price_num, price, Input.price1:Input.price3) %>%
  gather(answer_item, cost, Answer.dog_cost:Answer.sushi_cost)
```
